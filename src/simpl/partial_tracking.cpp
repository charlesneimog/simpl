...
PartialTracking::~PartialTracking() { clear(); }

void PartialTracking::clear() { _frames.clear(); }

int PartialTracking::sampling_rate() { return _sampling_rate; }

void PartialTracking::sampling_rate(int new_sampling_rate) { _sampling_rate = new_sampling_rate; }

int PartialTracking::max_partials() { return _max_partials; }

void PartialTracking::max_partials(int new_max_partials) { _max_partials = new_max_partials; }

int PartialTracking::min_partial_length() { return _min_partial_length; }

void PartialTracking::min_partial_length(int new_min_partial_length) {
    _min_partial_length = new_min_partial_length;
}

int PartialTracking::max_gap() { return _max_gap; }

void PartialTracking::max_gap(int new_max_gap) { _max_gap = new_max_gap; }
...
void MQPartialTracking::update_partials(Frame *frame) {
    ...
    while (partials && partials->peak && (num_partials < _max_partials)) {
        frame->add_partial(partials->peak->amplitude, partials->peak->frequency,
                           partials->peak->phase, 0.0);
...
SMSPartialTracking::SMSPartialTracking() {
    ...
    sms_initAnalysis(&_analysis_params, &_sound_header);
    char _headerStr[] = "simpl_peaks";

    sms_fillHeader(&_header, &_analysis_params, _headerStr);
...
void SMSPartialTracking::init_peaks() {
    ...
    _peak_amplitude = new s_sample[_max_partials];
    _peak_frequency = new s_sample[_max_partials];
    _peak_phase = new s_sample[_max_partials];

    memset(_peak_amplitude, 0.0, sizeof(s_sample) * _max_partials);
    memset(_peak_frequency, 0.0, sizeof(s_sample) * _max_partials);
    memset(_peak_phase, 0.0, sizeof(s_sample) * _max_partials);
}
...
void SMSPartialTracking::max_partials(int new_max_partials) {
    ...
    char _headerStr[] = "simpl_partials";
    sms_fillHeader(&_header, &_analysis_params, _headerStr);
    sms_allocFrameH(&_header, &_data);
...
// bool SMSPartialTracking::realtime() { return _analysis_params.realtime == 1; }
//
...
bool SMSPartialTracking::harmonic() { return _analysis_params.iFormat == SMS_FORMAT_HP; }
...
double SMSPartialTracking::default_fundamental() { return _analysis_params.fDefaultFundamental; }
...
int SMSPartialTracking::max_frame_delay() { return _analysis_params.iMaxDelayFrames; }
...
int SMSPartialTracking::analysis_delay() { return _analysis_params.analDelay; }
...
int SMSPartialTracking::min_good_frames() { return _analysis_params.minGoodFrames; }
...
bool SMSPartialTracking::clean_tracks() { return _analysis_params.iCleanTracks == 1; }
...
void SMSPartialTracking::update_partials(Frame *frame) {
    ...
    sms_setPeaks(&_analysis_params, _max_partials, _peak_amplitude, _max_partials, _peak_frequency,
    ...
    for (int i = 0; i < _data.nTracks; i++) {
        frame->add_partial(_data.pFSinAmp[i], _data.pFSinFreq[i], _data.pFSinPha[i], 0.0);
    }
}
...
void SndObjPartialTracking::update_partials(Frame *frame) {
    ...
    _analysis->SetPeaks(_max_partials, _peak_amplitude, _max_partials, _peak_frequency,
                        _max_partials, _peak_phase);
    ...
    for (int i = 0; i < num_partials; i++) {
        frame->add_partial(_analysis->Output(i * 3), _analysis->Output((i * 3) + 1),
                           _analysis->Output((i * 3) + 2), 0.0);
    }
...
TODO: Fix LorisPartialTracking

// ---------------------------------------------------------------------------
// LorisPartialTracking
// ---------------------------------------------------------------------------
SimplLorisPTAnalyzer::SimplLorisPTAnalyzer(int max_partials) : Loris::Analyzer(50, 100), _env(1.0) {
    buildFundamentalEnv(false);
    _partial_builder = new Loris::PartialBuilder(m_freqDrift);
    _partial_builder->maxPartials(max_partials);
}

SimplLorisPTAnalyzer::~SimplLorisPTAnalyzer() { delete _partial_builder; }
...
void LorisPartialTracking::update_partials(Frame *frame) {
    ...
    for (int i = 0; i < num_peaks; i++) {
        Loris::Breakpoint bp =
            Loris::Breakpoint(frame->peak(i)->frequency, frame->peak(i)->amplitude,
                              frame->peak(i)->bandwidth, frame->peak(i)->phase);
    ...
    for (int i = 0; i < num_partials; i++) {
        frame->add_partial(_analyzer->partials[i].amplitude(), _analyzer->partials[i].frequency(),
                           _analyzer->partials[i].phase(), _analyzer->partials[i].bandwidth());
    }
}
