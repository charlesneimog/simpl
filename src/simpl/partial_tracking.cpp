...
void SMSPartialTracking::update_partials(Frame *frame) {
  ...
  sms_setPeaks(&_analysis_params, _max_partials, _peak_amplitude, _max_partials,
               _peak_frequency, _max_partials, _peak_phase);
  ...
  for (int i = 0; i < _data.nTracks; i++) {
    frame->add_partial(_data.pFSinAmp[i], _data.pFSinFreq[i], _data.pFSinPha[i],
                       0.0);
  }
}
...
void SndObjPartialTracking::update_partials(Frame *frame) {
  ...
  _analysis->SetPeaks(_max_partials, _peak_amplitude, _max_partials,
                      _peak_frequency, _max_partials, _peak_phase);
...
SimplLorisPTAnalyzer::SimplLorisPTAnalyzer(int max_partials)
    : Loris::Analyzer(50, 100), _env(1.0) {
...
void LorisPartialTracking::update_partials(Frame *frame) {
  ...
  for (int i = 0; i < num_partials; i++) {
    frame->add_partial(
        _analyzer->partials[i].amplitude(), _analyzer->partials[i].frequency(),
