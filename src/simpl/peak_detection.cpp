...
void MQPeakDetection::find_peaks_in_frame(Frame *frame) {
    ...
    while (pl && pl->peak && num_peaks < _max_peaks) {
        frame->add_peak(pl->peak->amplitude, pl->peak->frequency,
                        pl->peak->phase, 0.0);
        pl = pl->next;
        num_peaks++;
    }
...
SimplLorisAnalyzer::SimplLorisAnalyzer(int window_size, s_sample resolution,
    ...
    Loris::KaiserWindow::buildTimeDerivativeWindow(_window_deriv,
                                                   _window_shape);
...
void SimplLorisAnalyzer::analyze(int audio_size, s_sample *audio) {
    ...
    fixBandwidth(peaks);
    if (m_bwAssocParam > 0) {
        _bw_associator->associateBandwidth(peaks.begin(), peaks.end(),
                                           peaks.end());
    }
}
