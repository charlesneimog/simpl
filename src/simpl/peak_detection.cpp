...
PeakDetection::~PeakDetection() { clear(); }
...
int PeakDetection::sampling_rate() { return _sampling_rate; }

void PeakDetection::sampling_rate(int new_sampling_rate) {
  _sampling_rate = new_sampling_rate;
}

int PeakDetection::frame_size() { return _frame_size; }

void PeakDetection::frame_size(int new_frame_size) {
  _frame_size = new_frame_size;
}

bool PeakDetection::static_frame_size() { return _static_frame_size; }

void PeakDetection::static_frame_size(bool new_static_frame_size) {
  _static_frame_size = new_static_frame_size;
}

int PeakDetection::next_frame_size() { return _frame_size; }

int PeakDetection::hop_size() { return _hop_size; }

void PeakDetection::hop_size(int new_hop_size) { _hop_size = new_hop_size; }

int PeakDetection::max_peaks() { return _max_peaks; }

void PeakDetection::max_peaks(int new_max_peaks) { _max_peaks = new_max_peaks; }

std::string PeakDetection::window_type() { return _window_type; }

void PeakDetection::window_type(std::string new_window_type) {
  _window_type = new_window_type;
}

int PeakDetection::window_size() { return _window_size; }

void PeakDetection::window_size(int new_window_size) {
  _window_size = new_window_size;
}

s_sample PeakDetection::min_peak_separation() { return _min_peak_separation; }

void PeakDetection::min_peak_separation(s_sample new_min_peak_separation) {
  _min_peak_separation = new_min_peak_separation;
}

int PeakDetection::num_frames() { return _frames.size(); }

Frame *PeakDetection::frame(int frame_number) { return _frames[frame_number]; }

Frames PeakDetection::frames() { return _frames; }

void PeakDetection::frames(Frames new_frames) { _frames = new_frames; }
...
Frames PeakDetection::find_peaks(int audio_size, s_sample *audio) {
  clear();
  unsigned int pos = 0;
...
MQPeakDetection::MQPeakDetection() {
  ...
  _mq_params.fundamental = _sampling_rate / (float)_frame_size;
  init_mq(&_mq_params);
}

MQPeakDetection::~MQPeakDetection() { destroy_mq(&_mq_params); }
...
void MQPeakDetection::reset() {
  ...
  _mq_params.frame_size = _frame_size;
  _mq_params.num_bins = (_frame_size / 2) + 1;
  _mq_params.fundamental = (float)_sampling_rate / _frame_size;
  init_mq(&_mq_params);
}
...
void MQPeakDetection::hop_size(int new_hop_size) { _hop_size = new_hop_size; }
...
void MQPeakDetection::find_peaks_in_frame(Frame *frame) {
  ...
  while (pl && pl->peak && num_peaks < _max_peaks) {
    frame->add_peak(pl->peak->amplitude, pl->peak->frequency, pl->peak->phase,
                    0.0);
    pl = pl->next;
    num_peaks++;
  }
...
int SMSPeakDetection::realtime() { return _analysis_params.realtime; }
...
void SMSPeakDetection::find_peaks_in_frame(Frame *frame) {
  int num_peaks =
      sms_findPeaks(frame->size(), frame->audio(), &_analysis_params, &_peaks);

  for (int i = 0; i < num_peaks; i++) {
    frame->add_peak(_peaks.pSpectralPeaks[i].fMag,
                    _peaks.pSpectralPeaks[i].fFreq,
                    _peaks.pSpectralPeaks[i].fPhase, 0.0);
  }
}
...
Frames SMSPeakDetection::find_peaks(int audio_size, s_sample *audio) {
  clear();
  unsigned int pos = 0;
  bool alloc_memory_in_frame = true;
...
void SndObjPeakDetection::find_peaks_in_frame(Frame *frame) {
  ...
  for (int i = 0; i < num_peaks; i++) {
    frame->add_peak(_analysis->Output(i * 3), _analysis->Output((i * 3) + 1),
...
// LorisPeakDetection
// ---------------------------------------------------------------------------
SimplLorisAnalyzer::SimplLorisAnalyzer(int window_size, s_sample resolution,
                                       int hop_size, s_sample sampling_rate)
  ...
  if (m_bwAssocParam > 0) {
    _bw_associator.reset(
        new Loris::AssociateBandwidth(bwRegionWidth(), sampling_rate));
  }
}
...
void SimplLorisAnalyzer::analyze(int audio_size, s_sample *audio) {
...
void LorisPeakDetection::reset() {
  ...
  _analyzer = new SimplLorisAnalyzer(_frame_size, _resolution, _hop_size,
                                     _sampling_rate);
}
...
void LorisPeakDetection::find_peaks_in_frame(Frame *frame) {
  ...
  for (int i = 0; i < num_peaks; i++) {
    frame->add_peak(_analyzer->peaks[i].amplitude(),
                    _analyzer->peaks[i].frequency(), 0.0,
