...
public:
  ...
  virtual s_sample min_peak_separation();
  virtual void min_peak_separation(s_sample new_min_peak_separation);
  int num_frames();
  ...
  // each frame
  virtual Frames find_peaks(int audio_size, s_sample *audio);
};
...
public:
  ...
  void find_peaks_in_frame(Frame *frame);
  Frames find_peaks(int audio_size, s_sample *audio);
};
...
protected:
  s_sample _window_shape;
  std::vector<s_sample> _window;
  std::vector<s_sample> _window_deriv;
...
public:
  SimplLorisAnalyzer(int window_size, s_sample resolution, int hop_size,
                     s_sample sampling_rate);
  ~SimplLorisAnalyzer();
  Loris::Peaks peaks;
  void analyze(int audio_size, s_sample *audio);
};
