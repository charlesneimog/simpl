...
int Residual::frame_size() { return _frame_size; }

void Residual::frame_size(int new_frame_size) { _frame_size = new_frame_size; }

int Residual::hop_size() { return _hop_size; }

void Residual::hop_size(int new_hop_size) { _hop_size = new_hop_size; }

int Residual::sampling_rate() { return _sampling_rate; }
...
void Residual::find_residual(int synth_size, s_sample *synth, int original_size,
                             s_sample *original, int residual_size,
                             s_sample *residual) {}
...
Frames Residual::synth(int original_size, s_sample *original) {
  clear();
  unsigned int pos = 0;
...
int SMSResidual::num_stochastic_coeffs() { return _residual_params.nCoeffs; }
...
void SMSResidual::residual_frame(Frame *frame) {
  ...
  sms_findResidual(_hop_size, frame->synth(), _hop_size,
                   &(frame->audio()[frame->size() - _hop_size]),
                   &_residual_params);
...
void SMSResidual::synth_frame(Frame *frame) {
  ...
  sms_approxResidual(_hop_size, frame->residual(), _hop_size,
                     frame->synth_residual(), &_residual_params);
