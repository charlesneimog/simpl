...
void sms_analyzeFrame(int iCurrentFrame, SMS_AnalParams *pAnalParams, sfloat fRefFundamental) {
    ...
    if (pAnalParams->realtime == 0) {
        ...
        pFData = &(pAnalParams->soundBuffer.pFBuffer[iSoundLoc - pAnalParams->soundBuffer.iMarker]);
    } else {
        pFData =
            &(pAnalParams->soundBuffer.pFBuffer[pAnalParams->soundBuffer.sizeBuffer - sizeWindow]);
    }
    ...
    sms_spectrum(sizeWindow, pFData, pAnalParams->spectrumWindow, sizeMag, pAnalParams->magSpectrum,
                 pAnalParams->phaseSpectrum, pAnalParams->fftBuffer);
    ...
    pCurrentFrame->nPeaks =
        sms_detectPeaks(sizeMag, pAnalParams->magSpectrum, pAnalParams->phaseSpectrum,
                        pCurrentFrame->pSpectralPeaks, pAnalParams);

    /* find a reference harmonic */
    if (pCurrentFrame->nPeaks > 0 &&
        (pAnalParams->iFormat == SMS_FORMAT_H || pAnalParams->iFormat == SMS_FORMAT_HP)) {
        pCurrentFrame->fFundamental =
            sms_harmDetection(pAnalParams->maxPeaks, pCurrentFrame->pSpectralPeaks, fRefFundamental,
                              pAnalParams->iRefHarmonic, pAnalParams->fLowestFundamental,
                              pAnalParams->fHighestFundamental, pAnalParams->iSoundType,
                              pAnalParams->fMinRefHarmMag, pAnalParams->fRefHarmMagDiffFromMax);
    }
}
...
static int ReAnalyzeFrame(int iCurrentFrame, SMS_AnalParams *pAnalParams) {
    ...
    if (fAvgDeviation <= pAnalParams->maxDeviation) {
        for (i = 0; i < pAnalParams->analDelay; i++) {
            ...
            iNewFrameSize =
                ((pAnalParams->iSamplingRate / fLastFund) * pAnalParams->fSizeWindow / 2) * 2 + 1;

            if (fFund <= 0 || fDev > .2 ||
                fabs((double)(pAnalParams->ppFrames[iFirstFrame - i]->iFrameSize - iNewFrameSize)) /
                        iNewFrameSize >=
                    .2) {
...
int sms_findPeaks(int sizeWaveform, sfloat *pWaveform, SMS_AnalParams *pAnalParams,
...
void sms_setPeaks(SMS_AnalParams *pAnalParams, int numamps, sfloat *amps, int numfreqs,
                  sfloat *freqs, int numphases, sfloat *phases) {
    ...
    if (frame->nPeaks > 0 &&
        ...
        frame->fFundamental =
            sms_harmDetection(frame->nPeaks, frame->pSpectralPeaks, refFundamental,
                              pAnalParams->iRefHarmonic, pAnalParams->fLowestFundamental,
                              pAnalParams->fHighestFundamental, pAnalParams->iSoundType,
                              pAnalParams->fMinRefHarmMag, pAnalParams->fRefHarmMagDiffFromMax);
    }
...
int sms_findPartials(SMS_Data *pSmsData, SMS_AnalParams *pAnalParams) {
    ...
    memcpy((char *)pSmsData->pFSinFreq, (char *)pAnalParams->ppFrames[0]->deterministic.pFSinFreq,
           length);
    memcpy((char *)pSmsData->pFSinAmp, (char *)pAnalParams->ppFrames[0]->deterministic.pFSinAmp,
           length);
    ...
    if (pAnalParams->iFormat == SMS_FORMAT_HP || pAnalParams->iFormat == SMS_FORMAT_IHP) {
        memcpy((char *)pSmsData->pFSinPha, (char *)pAnalParams->ppFrames[0]->deterministic.pFSinPha,
               length);
    }
...
int sms_findResidual(int sizeSynthesis, sfloat *pSynthesis, int sizeOriginal, sfloat *pOriginal,
    ...
    sms_filterHighPass(residualParams->hopSize, residualParams->residual,
...
int sms_analyze(int sizeWaveform, sfloat *pWaveform, SMS_Data *pSmsData,
    ...
    if (pAnalParams->iStochasticType != SMS_STOC_NONE) {
        ...
        if (pAnalParams->ppFrames[1]->iStatus != SMS_FRAME_EMPTY &&
            ...
            memset(pAnalParams->synthBuffer.pFBuffer + pAnalParams->sizeHop, 0,
                   sizeof(sfloat) * pAnalParams->sizeHop);
        ...
        if (pAnalParams->ppFrames[0]->iStatus != SMS_FRAME_EMPTY &&
            ...
            /* obtain residual sound from original and synthesized sounds.  accumulate the residual
             * percentage.*/
            pAnalParams->fResidualAccumPerc +=
                sms_residual(sizeData, pAnalParams->synthBuffer.pFBuffer, pOriginal,
            ...
            if (pAnalParams->iStochasticType == SMS_STOC_APPROX) {
                ...
                sms_filterHighPass(sizeData, pAnalParams->residualParams.residual,
                                   pAnalParams->iSamplingRate);

                /* approximate residual */
                sms_stocAnalysis(sizeData, pAnalParams->residualParams.residual,
                                 pAnalParams->residualParams.fftWindow, pSmsData, pAnalParams);
            } else if (pAnalParams->iStochasticType == SMS_STOC_IFFT) {
                int sizeMag = sms_power2(sizeData >> 1);
                sms_spectrum(sizeData, pAnalParams->residualParams.residual,
                             pAnalParams->residualParams.fftWindow, sizeMag, pSmsData->pFStocCoeff,
                             pSmsData->pResPhase, pAnalParams->fftBuffer);
            }

            /* get sharper transitions in deterministic representation */
            sms_scaleDet(pAnalParams->synthBuffer.pFBuffer, pOriginal,
                         pAnalParams->ppFrames[0]->deterministic.pFSinAmp, pAnalParams,
                         pSmsData->nTracks);
    ...
    /* return analysis data */
        ...
        int length = sizeof(sfloat) * pSmsData->nTracks;
        memcpy((char *)pSmsData->pFSinFreq,
               (char *)pAnalParams->ppFrames[0]->deterministic.pFSinFreq, length);
        memcpy((char *)pSmsData->pFSinAmp, (char *)pAnalParams->ppFrames[0]->deterministic.pFSinAmp,
               length);
        ...
        sms_arrayDBToMag(pSmsData->nTracks, pSmsData->pFSinAmp);
        if (pAnalParams->iFormat == SMS_FORMAT_HP || pAnalParams->iFormat == SMS_FORMAT_IHP)
            memcpy((char *)pSmsData->pFSinPha,
                   (char *)pAnalParams->ppFrames[0]->deterministic.pFSinPha, length);
