...
static int FindNextMax(sfloat *pFMagSpectrum, int iHighBinBound,
  ...
  sfloat fNextVal =
      (iCurrentBin >= iHighBinBound) ? 0 : pFMagSpectrum[iCurrentBin + 1];

  /* try to find a local maximum */
  while (iCurrentBin <= iHighBinBound) {
    if (fCurrentVal > fMinPeakMag && fCurrentVal >= fPrevVal &&
...
static int FindNextPeak(sfloat *pFMagSpectrum, int iHighestBin,
  ...
  while ((iPeakBin = FindNextMax(pFMagSpectrum, iHighestBin, pICurrentLoc,
                                 &fPeakMag, fMinPeakMag)) <= iHighestBin) {
    ...
    *pFPeakMag =
        PeakInterpolation(fPeakMag, fLeftBinVal, fRightBinVal, &fDiffFromMax);
...
static sfloat GetPhaseVal(sfloat *pPhaseSpectrum, sfloat fPeakLoc) {
  ...
  sfloat fFraction = fPeakLoc - bin, fLeftPha = pPhaseSpectrum[bin],
...
int sms_detectPeaks(int sizeSpec, sfloat *pMag, sfloat *pPhase,
  ...
  int iFirstBin =
      MAX(1, sizeFft * pAnalParams->fLowestFreq / pAnalParams->iSamplingRate);
  int iHighestBin = MIN(sizeSpec - 1, sizeFft * pAnalParams->fHighestFreq /
  ...
  while ((iPeak < pAnalParams->maxPeaks) &&
         (FindNextPeak(pMag, iHighestBin, &iCurrentLoc, &fPeakMag, &fPeakLoc,
                       pAnalParams->fMinPeakMag) == 1)) {
    /* store peak values */
    pSpectralPeaks[iPeak].fFreq =
        pAnalParams->iSamplingRate * fPeakLoc * fInvSizeFft;
