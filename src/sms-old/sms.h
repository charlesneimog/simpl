 ...
 *
 *  The first four members of the Header are necessary in this order to
 * correctly open the .sms files created by this library.
 *
 ...
 *
 *  In the first release, the descriptors are not used, but are here because
 * they were implemented in previous versions of this code (in the 90's).  With
 * time, the documentation will be updated to reflect which members of the
 * header are useful in manipulations, and what functions to use for these
 * manipulatinos
 */
typedef struct SMSHeader {
  ...
  int iSamplingRate;   /*!< samplerate of analysis signal (necessary to recreate
                          residual spectrum */
  ...
  int iMaxFreq; /*!< maximum frequency of peaks (also corresponds to the last
                   bin of the specEnv */
 ...
 *
 * Here is where all the analysis data ends up. Once data is in here, it is
 * ready for synthesis.
 *
 * It is in one contigous block (pSmsData), the other pointer members point
 * specifically to each component in the block.
 *
 * pFSinPha is optional in the final output, but it is always used to construct
 * the residual signal.
 */
...
typedef struct {
  ...
  int iStatus; /*!< status of frame enumerated by SMS_FRAME_STATUS \see
                  SMS_FRAME_STATUS */
} SMS_AnalFrame;
 ...
 *
 * This structure contains all the necessary settings and memory for residual
 * synthesis.
 *
 */
 ...
 * and peak detection/continuation process can be re-computed with more
 * accuracy.
 *
 */
typedef struct SMSAnalysisParams {
  ...
  int iStochasticType;  /*!< type of stochastic model defined by
                           SMS_StocSynthType \see SMS_StocSynthType */
  int iFrameRate;       /*!< rate in Hz of data frames */
  int nStochasticCoeff; /*!< number of stochastic coefficients per frame  */
  ...
  sfloat fPeakContToGuide; /*!< contribution of previous peak to current guide
                              (between 0 and 1) */
  sfloat fFundContToGuide; /*!< contribution of current fundamental to current
  ...
  int nGuides; /*!< number of guides used for peak detection and continuation
                  \see SMS_Guide */
  ...
  sfloat fRefHarmMagDiffFromMax; /*!< maximum magnitude difference from
  ...
  int iMaxDelayFrames;    /*!< maximum number of frames to delay before peak
                             continuation */
  int minGoodFrames; /*!< minimum number of stable frames for backward search */
  sfloat maxDeviation; /*!< maximum deviation allowed */
  int analDelay;       /*! number of frames in the past to be looked in possible
                          re-analyze */
  ...
  SMS_SndBuffer
      synthBuffer; /*!< resynthesized signal used to create the residual */
  SMS_AnalFrame
      *pFrames; /*!< an array of frames that have already been analyzed */
  ...
  SMS_AnalFrame **ppFrames; /*!< pointers to the frames analyzed (it is
                               circular-shifted once the array is full */
} SMS_AnalParams;

/*! \struct SMS_ModifyParams
 *
 * \brief structure with parameters and data that will be used to modify an
 * SMS_Data frame
 */
typedef struct {
  ...
  int doSinEnv; /*!< whether or not to apply a new spectral envelope to the sin
                   component */
  sfloat sinEnvInterp; /*!< value between 0 (use frame's env) and 1 (use *env).
                          Interpolates inbetween values*/
  int sizeSinEnv;      /*!< size of the envelope pointed to by env */
  sfloat *sinEnv;      /*!< sinusoidal spectral envelope  */
  int doResEnv; /*!< whether or not to apply a new spectral envelope to the
                   residual component */
  sfloat resEnvInterp; /*!< value between 0 (use frame's env) and 1 (use *env).
                          Interpolates inbetween values*/
 ...
 *
 * This structure contains all the necessary settings for different types of
 * synthesis. It also holds arrays for windows and the inverse-FFT, as well as
 * the previously synthesized frame.
 *
 */
typedef struct SMSSynthParams {
  ...
  int iDetSynthType;   /*!< method for synthesizing deterministic component \see
                          SMS_DetSynthType */
  int iOriginalSRate;  /*!< samplerate of the sound model source (for stochastic
                          synthesis approximation) */
  int iSamplingRate;   /*!< synthesis samplerate */
  int sizeHop;         /*!< number of samples to synthesis for each frame */
  int origSizeHop; /*!< original number of samples used to create each analysis
                      frame */
  int nTracks;
  int nStochasticCoeff;
  int deEmphasis; /*!< whether or not to perform de-emphasis */
  sfloat deEmphasisLastValue;
  sfloat *pFDetWindow;  /*!< array to hold the window used for deterministic
                           synthesis  \see SMS_WIN_IFFT */
  sfloat *pFStocWindow; /*!< array to hold the window used for stochastic
                           synthesis (Hanning) */
  sfloat *pSynthBuff;   /*!< an array for keeping samples during overlap-add (2x
                           sizeHop) */
  sfloat *pMagBuff; /*!< an array for keeping magnitude spectrum for stochastic
                       synthesis */
  sfloat *pPhaseBuff; /*!< an array for keeping phase spectrum for stochastic
                         synthesis */
  sfloat *pSpectra;   /*!< array for in-place FFT transform */
  SMS_Data
      prevFrame; /*!< previous data frame, for interpolation between frames */
 ...
 *
 * Is the signal is known to be harmonic, using format harmonic (with out
 * without phase) will give more accuracy to the peak continuation algorithm. If
 * the signal is known to be inharmonic, then it is best to use one of the
 * inharmonic settings to tell the peak continuation algorithm to just look at
 * the peaks and connect them, instead of trying to look for peaks at specific
 * frequencies (harmonic partials).
 */
enum SMS_Format {
 ...
 *
 * The Inverse FFT method is not implemented, but is based on the idea of
 * storing the exact spectrum and phases of the residual component to file.
 * Synthesis could then be an exact reconstruction of the original signal,
 * provided interpolation is not necessary.
 *
...
enum SMS_StocSynthType {
  ...
  SMS_STOC_APPROX, /*!< 1, Inverse FFT, magnitude approximation and generated
                      phases */
...
enum SMS_DBG {
  ...
  SMS_DBG_DET, /*!< 1, not yet implemented \todo make this show main information
                  to look at for  discovering the correct deterministic
                  parameters*/
...
enum SMS_WINDOWS {
  ...
  SMS_WIN_IFFT     /*!< 6: window for deterministic synthesis based on the
                      Inverse-FFT algorithm.     This is a combination of an inverse
                      Blackman-Harris 92dB and a triangular window. */
};
...
int sms_findPeaks(int sizeWaveform, sfloat *pWaveform,
                  ...
                  SMS_SpectralPeaks *pSpectralPeaks);
int sms_findPartials(SMS_Data *pSmsFrame, SMS_AnalParams *pAnalParams);
int sms_findResidual(int sizeSynthesis, sfloat *pSynthesis, int sizeOriginal,
                     sfloat *pOriginal, SMS_ResidualParams *residualParams);
void sms_approxResidual(int sizeResidual, sfloat *residual, int sizeApprox,
                        sfloat *approx, SMS_ResidualParams *residualParams);
int sms_analyze(int sizeWaveform, sfloat *pWaveform, SMS_Data *pSmsData,
                SMS_AnalParams *pAnalParams);
void sms_analyzeFrame(int iCurrentFrame, SMS_AnalParams *pAnalParams,
...
void sms_fillSoundBuffer(int sizeWaveform, sfloat *pWaveform,
                         SMS_AnalParams *pAnalParams);
...
int sms_spectrum(int sizeWindow, sfloat *pWaveform, sfloat *pWindow,
                 int sizeMag, sfloat *pMag, sfloat *pPhase, sfloat *pFftBuffer);
int sms_spectrumW(int sizeWindow, sfloat *pWaveform, sfloat *pWindow,
...
void sms_dCepstrum(int sizeCepstrum, sfloat *pCepstrum, int sizeFreq,
                   sfloat *pFreq, sfloat *pMag, sfloat fLambda,
                   int iSamplingRate);
void sms_dCepstrumEnvelope(int sizeCepstrum, sfloat *pCepstrum, int sizeEnv,
                           sfloat *pEnv);
...
sfloat sms_harmDetection(int numPeaks, SMS_Peak *spectralPeaks,
                         sfloat refFundamental, sfloat refHarmonic,
                         sfloat lowestFreq, sfloat highestFreq, int soundType,
                         sfloat minRefHarmMag, sfloat refHarmMagDiffFromMax);
...
void sms_sineSynthFrame(SMS_Data *pSmsFrame, sfloat *pBuffer, int sizeBuffer,
                        SMS_Data *pLastFrame, int iSamplingRate);
...
void sms_writeDebugData(sfloat *pBuffer1, sfloat *pBuffer2, sfloat *pBuffer3,
                        int sizeBuffer);
